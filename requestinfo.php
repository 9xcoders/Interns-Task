...
if(isset($_POST['submit'])){
	//echo"Request Info Submit";
	$success="";
	$error="";
	if(!empty($_POST['name'])  && !empty($_POST['email']) && !empty($_POST['mobile']) && !empty($_POST['birthdate']) &&  !empty($_POST['gender']) && !empty($_POST['mladdress']) && !empty($_POST['country']) && !empty($_POST['postal']) && !empty($_POST['message'])){
	 	$name = $_POST['name'];
	 	$email = $_POST['email'];
		$mobile = $_POST['mobile'];
		$birthdate = $_POST['birthdate'];
		$gender = $_POST['gender'];
    $address = $_POST['mladdress'];
    $country = $_POST['country'];
    $postal = $_POST['postal'];
	 	$msg = $_POST['message'];

		$info_in = "INSERT INTO req_info (name, email, mobile_no, date, gender, address, country, postal_code, message) VALUES ('$name', '$email', $mobile, '$birthdate', '$gender', '$address', '$country', $postal,'$msg')";
		
		if(mysqli_query($conn, $info_in)){
							...
							<div class="row">
								<div class="col-sm-12 text-center inner">
									<div class="contact-content">
										...
										<div class="row">                            
											<div class="col-sm-12">
												<p>Please complete the form below to receive updates about our admissions process, our world-class academics, our adventurous and inclusive students and all things</p>                         
											</div>
										</div>
                    <?php
											if(!empty($success)){
                          ...
                          <textarea name="mladdress" rows="4" class="form-control" id="mladdress" placeholder="Mailing Address"></textarea><br>
                          <input name="country" type="text" class="form-control" id="country" placeholder="Country">
                          <input name="postal" type="text" class="form-control" id="postal" placeholder="postal code">
													
												</div>
											</div>
											<div class="col-sm-6 contact-form-right">
												<div class="form-group">
													<textarea name="message" rows="6" class="form-control" id="comment" placeholder="Your message here...(Interests...)"></textarea>
